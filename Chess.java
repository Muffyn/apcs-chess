...
public class Chess extends JPanel {
	...
	public static void drawMarker(Graphics g, Position position, Color color, int length, int translation) {
		g.setColor(color);
		g.fillRect(length * position.getX() + translation, length * position.getY(), (length - 1) / MARKER_SCALE + 1,
				(length - 1) / MARKER_SCALE + 1);
	}
	...
	public static void drawPieces(ArrayList<Piece> pieces, Graphics g, String color, int length, int translation) {
		pieces.forEach(piece -> {
			...
			g.drawImage(img, length * piece.getX() + translation + length / PIECE_SCALE,
					length * piece.getY() + length / PIECE_SCALE, (length - 1) - length / (PIECE_SCALE / 2),
					(length - 1) - length / (PIECE_SCALE / 2), null);
		});
	}
	...
	public Chess(boolean multiplayer) {
		...
		player.getPieces()
				.forEach(piece -> piece.setPossiblePositions(player.getPieces(), player.getNext().getPieces()));
